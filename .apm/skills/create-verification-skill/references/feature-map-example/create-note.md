...
- `control-notes doctor` reports the expected URL and disposable data
  directory.
...
- Pressing `n` while a textbox has focus types the character instead of opening
  a new editor.
...
- Remove `Release checklist` and `CLI note` during fixture cleanup, but retain
  their proof artifacts.
