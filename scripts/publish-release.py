...
def publish(marketplace, remote, tag, channel="codex"):
    ...
    with tempfile.TemporaryDirectory(prefix="lachie-publish-") as temp:
        repo = Path(temp)
        run("git", "init", f"--initial-branch={channel}", cwd=repo)
        ...
        refs = run("git", "ls-remote", "origin", f"refs/heads/{channel}", f"refs/tags/{channel}-{tag}", cwd=repo)
        if f"refs/tags/{channel}-{tag}" in refs:
            run("git", "fetch", "origin", f"refs/tags/{channel}-{tag}", cwd=repo)
            ...
            print(f"{channel}-{tag} is already published; leaving the release channel unchanged")
            return
        if f"refs/heads/{channel}" in refs:
            run("git", "fetch", "origin", f"refs/heads/{channel}", cwd=repo)
        ...
        run("git", "commit", "-m", f"Release {channel} plugin {tag} from {release['source_commit']}", cwd=repo)
        run("git", "tag", f"{channel}-{tag}", cwd=repo)
        run("git", "push", "--atomic", "origin", f"HEAD:refs/heads/{channel}", f"refs/tags/{channel}-{tag}", cwd=repo)
    print(f"Published {channel}-{tag} and advanced the {channel} release channel")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--channel", choices=("codex", "claude", "opencode2"), default="codex")
    args = parser.parse_args()
    publish(args.marketplace.resolve(), args.remote, args.tag, args.channel)
