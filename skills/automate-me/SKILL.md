---
name: automate-me
description: Turn Lachie's working conventions into a personal mode skill or refresh the existing one.
disable-model-invocation: true
---

# Automate Me

Turn recurring working conventions into one concise `lachie-mode` skill. Mine
available evidence, ask for intent, draft the skill, and let Lachie approve the
result.
...
recursively for `lachie-mode/SKILL.md`. Preserve an existing category and path.
...
For a new project skill, write `.agents/skills/lachie-mode/SKILL.md`. Use
`~/.agents/skills/lachie-mode/SKILL.md` only when Lachie chooses a user-local
...
- `name: lachie-mode`.
- a one-scalar description naming Lachie and the purpose of working in
  Lachie's style.
- `disable-model-invocation: true`, unless Lachie explicitly requests automatic
  invocation.
...
user" or "the human" in reusable imperatives while retaining Lachie in the
skill identity and invocation description.

### 6. Review and revise

Show the complete draft and ask whether it reads like Lachie, misses a durable
...
- Drop preferences that conflict across sessions until Lachie resolves them.
...
The selected `lachie-mode` path contains a valid, reviewed skill. Every rule is
