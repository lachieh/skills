---
name: interrogate
description: Run an evidence-based, multi-provider adversarial review of a concrete diff or artifact against its stated intent.
---

# Interrogate

Interrogate returns a boss-owned verdict on a concrete change. It does not apply
changes in report-only use. A calling implementation or `shipit` procedure may
act on accepted findings.

## Procedure

1. Determine the artifact, repository base, fixed point, surrounding context,
   and applicable standards. Discover the base branch rather than assuming it.
...
4. Give every reviewer the same intent, criteria, commit range, and source
   pointers. Avoid large inline payloads.
...
How Critique reviews an existing subsystem after explaining it. Arena compares
competing artifacts. Interrogate attacks one concrete change against its intent.
